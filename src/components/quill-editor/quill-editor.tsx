...
const QuillEditor: React.FC<QuillEditorProps> = ({
  ...
  const details = useMemo(() => {
    ...
    if (dirType === 'file') {
      selectedDir = state.workspaces
        ...
        ?.folders.find((folder: { id: string | undefined; }) => folder.id === folderId)
        ?.files.find((file: { id: string; }) => file.id === fileId);
    }
    if (dirType === 'folder') {
      selectedDir = state.workspaces
        .find((workspace) => workspace.id === workspaceId)
        ?.folders.find((folder: { id: string; }) => folder.id === fileId);
    }
  ...
  const breadCrumbs = useMemo(() => {
    ...
    const segments = pathname
      .split('/')
      .filter((val: string) => val !== 'dashboard' && val);
    ...
    const folderDetails = workspaceDetails?.folders.find(
      (folder: { id: any; }) => folder.id === folderSegment
    );
    ...
    const fileDetails = folderDetails?.files.find(
      (file: { id: any; }) => file.id === fileSegment
    );
          ...
          >
            ...
            >
              <span className="text-white">
               The {dirType} has been moved to the trash.
              </span>
              <Button
                size="sm"
                variant="outline"
